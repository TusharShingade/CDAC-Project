...
export default function ViewWalletCustomer() {
  ...
  const sendData = (e) => {
    ...
    axios.post('http://localhost:8080/updateWallet', data)
      .then(response => {
        ...
        navigate('/viewwalletcustomer') // Navigate to '/adminhome' route
  ...
  return (
   ...
   <div className="container-fluid mt-5 col-5 fw-bold border bg-white text-primary">
      <h1>Wallet Information</h1>
      <div>
        <h3 className='text-dark'> Welcome {user && user.fname}</h3>
        <h3 className='text-warning'> Amount :: {user && user.wallet}</h3>
      </div>
      <div>
        {/* <form>
          <div className="mb-3">
